...
fileprivate func GetBitInt(_ data: Int, _ length: Int, _ shift: Int) -> Int {
    return (data & (((1 << length) - 1) << shift)) >> shift
}

//1887--2110之间适用，如果需要更多，修改对应的数组数据
fileprivate func SolarToInt(_ y: Int, _ m: Int, _ d: Int) -> Int {
    let m = (m + 9) % 12
    let y = y - m / 10
...
public struct WXBSolarLunar {
    
    private static func solarFromInt(_ g: Int) -> Solar {
...
public extension WXBSolarLunar {
    ...
    static func solarFromLunar(_ lunar: Lunar) -> Solar {
        ...
        let solar11 = solar_1_1[lunar.year - solar_1_1[0]]
    ...
    ///公历转农历
    static func lunarFromSolar(_ solar: Solar) -> Lunar {
        ...
        var offset = SolarToInt(solar.year, solar.month, solar.day) - SolarToInt(y, m, d)
        ...
        let lunar = Lunar.init(y: lunarY, m: lunarM, d: lunarD, leap: isleap)
        return lunar
    }
    
    ///对应的星座计算
    func constellationFromSolar(_ solar: Solar) -> String {
        var constellation = ""
        let i_month = solar.month
        let i_day = solar.day
